...
function PanelContent({ label, className }) {
  return (
    <div className={`flex items-center justify-center h-full text-sm font-[500] text-[var(--invin-text-dim)] ${className || ''}`}>
      {label}
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        <p className="text-[length:var(--invin-text-eyebrow)] font-[600] uppercase tracking-[0.05em] text-[var(--invin-text-faint)]">ResizablePanelGroup</p>
        <PropsTable
          props={[
      ...
      <div className="space-y-4">
        <p className="text-[length:var(--invin-text-eyebrow)] font-[600] uppercase tracking-[0.05em] text-[var(--invin-text-faint)]">ResizablePanel</p>
        <PropsTable
          props={[
      ...
      <div className="space-y-4">
        <p className="text-[length:var(--invin-text-eyebrow)] font-[600] uppercase tracking-[0.05em] text-[var(--invin-text-faint)]">ResizableHandle</p>
        <PropsTable
          props={[
      ...
      >
        <div className="border border-[var(--invin-border)] rounded-xl overflow-hidden h-[200px]">
          <ResizablePanelGroup direction="horizontal">
            <ResizablePanel defaultSize={35} minSize={20}>
              <PanelContent label="Sidebar (35%)" className="bg-[var(--invin-surface-hover)]/50" />
            </ResizablePanel>
      ...
      >
        <div className="border border-[var(--invin-border)] rounded-xl overflow-hidden h-[300px]">
          <ResizablePanelGroup direction="vertical">
            <ResizablePanel defaultSize={40} minSize={15}>
              <PanelContent label="Top panel (40%)" className="bg-[var(--invin-surface-hover)]/50" />
            </ResizablePanel>
            <ResizableHandle withHandle />
        ...
        <div className="border border-[var(--invin-border)] rounded-xl overflow-hidden h-[200px]">
          <ResizablePanelGroup direction="horizontal">
            <ResizablePanel defaultSize={20} minSize={10} maxSize={40}>
              <PanelContent label="Nav" className="bg-[var(--invin-surface-hover)]/50" />
            </ResizablePanel>
            <ResizableHandle />
            ...
            <ResizablePanel defaultSize={25} minSize={15} maxSize={40}>
              <PanelContent label="Details" className="bg-[var(--invin-surface-hover)]/50" />
            </ResizablePanel>
          </ResizablePanelGroup>
        </div>
      </PlaygroundSection>
      ...
      >
        <div className="border border-[var(--invin-border)] rounded-xl overflow-hidden h-[320px]">
          <ResizablePanelGroup direction="horizontal">
            <ResizablePanel defaultSize={25} minSize={15}>
              <PanelContent label="Explorer" className="bg-[var(--invin-surface-hover)]/50" />
            </ResizablePanel>
            <ResizableHandle />
            <ResizablePanel defaultSize={75}>
              <ResizablePanelGroup direction="vertical">
                ...
                <ResizablePanel defaultSize={30} minSize={15}>
                  <PanelContent label="Terminal" className="bg-[var(--invin-surface-hover)]/50" />
                </ResizablePanel>
              </ResizablePanelGroup>
            </ResizablePanel>
          </ResizablePanelGroup>
      ...
      >
        <div className="border border-[var(--invin-border)] rounded-xl overflow-hidden h-[280px]">
          <ResizablePanelGroup direction="horizontal">
            <ResizablePanel defaultSize={35} minSize={25} maxSize={50}>
              <div className="h-full overflow-auto p-3 space-y-1">
                {['Security Alert: Failed login attempt', 'Weekly digest — Jul 8–12', 'Invoice #INV-2847 paid', 'New team member: Carol D.', 'Scan report ready', 'Compliance audit reminder'].map((subject, i) => (
                  <div key={i} className={`px-3 py-2 rounded-md cursor-pointer text-sm transition-colors ${i === 0 ? 'bg-[var(--invin-accent-soft)] text-[var(--invin-accent)]' : 'hover:bg-[var(--invin-surface-hover)] text-[var(--invin-text-dim)]'}`}>
                    <p className="font-[500] truncate">{subject}</p>
                    <p className="text-[10px] text-[var(--invin-text-faint)] mt-0.5">Jul {12 - i}</p>
                  </div>
                ))}
              </div>
            </ResizablePanel>
            ...
            <ResizablePanel defaultSize={65} minSize={35}>
              <div className="h-full p-4 space-y-3">
                <h3 className="text-sm font-[600]">Security Alert: Failed login attempt</h3>
                <p className="text-xs text-[var(--invin-text-dim)]">From: system@invin.io · Jul 12, 09:45 AM</p>
                <p className="text-sm text-[var(--invin-text-dim)] leading-relaxed">
